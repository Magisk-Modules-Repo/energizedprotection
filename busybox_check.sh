...
BBMAG=$MAGIMG/busybox-ndk/system/xbin/wget

if [[ -d "$BBPATH" && -h "$BBMAG" ]]; then
	...
	touch $mnt/install_done
	touch $imgmnt/busybox-ndk/install_done
	chmod 0777 $imgmnt/busybox-ndk/install_done
	chmod 0777 $mnt/install_done
